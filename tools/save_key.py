#!/usr/bin/env python3
"""Standalone save-key derivation tool.

Derives the local SQLCipher save key from a conventional 17-digit
SteamID64, using the same Rijndael-256 derivation as Solo Leveling: ARISE
OVERDRIVE. This script is fully self-contained (standard library only) and
does not depend on the rest of the SaveGameEditor package, so it can be
copied and run on its own.

Usage:
    python save_key.py <steam_id64>
    python save_key.py --steam-id 7656119XXXXXXXXXX
"""
...
class SaveKeyError(RuntimeError):
    """Raised when a save key cannot be derived from the given input."""
...
def derive_save_key(steam_id: str) -> str:
    ...
    if not steam_id.isascii() or not steam_id.isdecimal() or len(steam_id) != 17:
        raise SaveKeyError("Steam ID must be a 17-digit SteamID64.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Derive the local SQLCipher save key for a SteamID64.",
    )
    parser.add_argument(
        "steam_id",
        nargs="?",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        print(derive_save_key(steam_id))
    except SaveKeyError as error:
